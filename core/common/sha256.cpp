...
Sha256::Sha256() {
    for (int i = 0; i < 8; ++i) {
        state_[static_cast<std::size_t>(i)] = kInit[i];
    }
}
...
void Sha256::update(const void* data, std::size_t length) {
    ...
    while (length > 0) {
        const std::size_t take = std::min(length, buffer_.size() - bufferUsed_);
        std::memcpy(buffer_.data() + bufferUsed_, bytes, take);
        bufferUsed_ += take;
        bytes += take;
        length -= take;
...
std::array<std::uint8_t, 32> Sha256::digest() {
    ...
    const std::uint64_t bits = bitLength_; // captured before padding is appended
    const std::uint8_t pad = 0x80;
    update(&pad, 1);
    const std::uint8_t zero = 0x00;
    while (bufferUsed_ != 56) {
        update(&zero, 1);
    }
    std::uint8_t lenBytes[8];
    for (int i = 0; i < 8; ++i) {
        lenBytes[i] = static_cast<std::uint8_t>(bits >> (56 - i * 8));
    }
    std::memcpy(buffer_.data() + bufferUsed_, lenBytes, 8);
    processBlock(buffer_.data());
